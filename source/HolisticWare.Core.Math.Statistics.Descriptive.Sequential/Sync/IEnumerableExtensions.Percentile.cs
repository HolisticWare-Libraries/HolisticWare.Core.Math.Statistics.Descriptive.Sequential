...
namespace Core.Math.Statistics.Descriptive.Sequential
{
    ...
    public static partial class IEnumerableExtensionsPercentiles
    {
        ...
        //--------------------------------------------------------------------------------------------------------------
        public static double PercentileRank(this IEnumerable<int> x, int p)
        {
            if (p < 0 || p > 100)
            {
                throw new ArgumentOutOfRangeException($"Percentile must be in the range [0, 1] the vale was {p}");
            }
            int n = x.Count();
        ...
        public static (int lower, double interpolated, int upper) Percentile(this IEnumerable<int> x, int p)
        {
            if (p < 0  || p > 100)
            {
                throw new ArgumentOutOfRangeException($"Percentile must be in the range [0, 1] the vale was {p}");
